...
struct AddSetBWView: View {
    ...
    let weightOfUser = AppStorageDataManager.shared.getUserInfoFloat(key: "userWeight")
    ...
    var body: some View {
        VStack {
            ...
            Button {
                let newSet = Set(id: UUID().uuidString, weight: weightOfUser, amount: Int(amount) ?? 0)
